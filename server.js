...
app.get('/test/:id', async (req, res) => {
  ...
  fetch(`https://apis.accela.com/v4/records/${req.params.id}/addresses`, newOptions)
    ...
    .then(result => {
      if (result.status !== 200) {
        res.render('err');
        return;
      } else { res.render(`show`, { result: result.result }); }
    })
...
app.get('/test', async (req, res) => {
  ...
  fetch("https://apis.accela.com/v4/records/", newOptions)
    .then(response => response.json())
    // dump the response to the console
    .then(result => { console.log(result.result[0]); return result; })
    // if call is not success, render error page
    .then(result => {
      if (result.status !== 200) {
        res.render('err');
